with 
  qryMatches as ( 
    SELECT * FROM `dbt-demos.538_football.stg_football_matches` where league = 'Barclays Premier League'
    ),
  qryRankings as (
    SELECT * FROM `dbt-demos.538_football.stg_football_rankings` where league = 'Barclays Premier League'
  ),
